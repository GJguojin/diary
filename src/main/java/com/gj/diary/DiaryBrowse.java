...
//		}
	...
	private void getDiaryMessage() {
		...
		if (DiaryUtil.password == "" || DiaryUtil.password == null) {
			...
			String password = showInputDialog("请输入日记解密密码");
			if (password == null) {
				return;
			}
	...
	private void decryptPhoto() {
		try {
			...
			String password = showInputDialog("请输入照片解密密码");
			if (password == null) {
				return;
			}
	...
	public static String showInputDialog(String text) {
		...
		JOptionPane.showMessageDialog(null, pw, text, JOptionPane.QUESTION_MESSAGE);
